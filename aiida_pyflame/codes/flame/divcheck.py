...
def divcheck_report():
    wf_divcheck_group = Group.get(label='wf_divcheck')
    for a_wf_node in wf_divcheck_group.nodes:
        ...
        if a_node.is_finished_ok:
            ...
            if 'bulk' in a_node.label:
                log_write('Diversity check for bulk structures with {} atoms: #posin {}, #posout {}'.format(nat, nposin, nposout)+'\n')
            if 'cluster' in a_node.label:
                log_write('Diversity check for clusters with {} atoms: #posin {}, #posout {}'.format(nat, nposin, nposout)+'\n')
...
def get_bulk_structures(cycle_number, nat):
    ...
    for c_no_i in range(1, c_no+1):
        if c_no_i == c_no and len(poslow_structures) >= 10000:
            log_write('>>> WARNING: number of poslow bulk structures with {} atoms from previous cycles is larger than 10000 <<<'.format(nat)+'\n')
            return None, None
        ...
        if os.path.exists(fpath):
            ...
            if nat in poslows.keys():
                ...
                log_write('>>> WARNING: no poslow bulk structure with {} atoms (generated with minhocao) from cycle-{} is available <<<'.format(nat, str(c_no_i))+'\n')
        else:
            log_write('>>> WARNING: no poslow bulk structure (generated with minhocao) from cycle-{} is available <<<'.format(str(c_no_i))+'\n')
        ...
        if os.path.exists(fpath):
            ...
            if nat in poslows.keys():
                ...
                log_write('>>> WARNING: no poslow bulk structure with {} atoms (generated with bulk minhopp) from cycle-{} is available <<<'.format(nat, str(c_no_i))+'\n')
        else:
            log_write('>>> WARNING: no poslow bulk structure (generated with bulk minhopp) from cycle-{} is available <<<'.format(str(c_no_i))+'\n')
    ...
    if os.path.exists(fpath):
        ...
        if nat in posmds.keys():
            ...
            log_write('>>> WARNING: no bulk structure with {} atoms (generated with minhocao) from {} is available <<<'.format(nat, str(cycle_number))+'\n')
    else:
        log_write('>>> WARNING: no trajectory (generated with minhocao) from {} is available <<<'.format(str(cycle_number))+'\n')
    ...
    if os.path.exists(fpath):
        ...
        if nat in trajs.keys():
            ...
            log_write('>>> WARNING: no bulk structure with {} atoms (generated with bulk minhopp) from {} is available <<<'.format(nat, str(cycle_number))+'\n')
    else:
        log_write('>>> WARNING: no trajectory (generated with bulk minhopp) from {} is available <<<'.format(str(cycle_number))+'\n')
...
def get_cluster_structures(cycle_number, nat):
    ...
    for c_no_i in range(1, c_no+1):
        if c_no_i == c_no and len(poslow_structures) >= 10000:
            log_write('>>> WARNING: number of poslow cluster structures with {} atoms from previous cycles is larger than 10000 <<<'.format(nat)+'\n')
            return None, None
        ...
        if os.path.exists(fpath):
            ...
            if nat in poslows.keys():
                ...
                log_write('>>> WARNING: no poslow cluster structure with {} atoms (generated with cluster minhopp) from cycle-{} is available <<<'.format(nat, str(c_no_i))+'\n')
        else:
            log_write('>>> WARNING: no poslow cluster structure (generated with cluster minhopp) from cycle-{} is available <<<'.format(str(c_no_i))+'\n')
    ...
    if os.path.exists(fpath):
        ...
        if nat in trajs.keys():
            ...
            log_write('>>> WARNING: no cluster structure with {} atoms (generated with cluster minhopp) from {} is available <<<'.format(nat, str(cycle_number))+'\n')
    else:
        log_write('>>> WARNING: no trajectory from {} is available <<<'.format(str(cycle_number))+'\n')
