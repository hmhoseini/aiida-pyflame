...
from aiida.orm import Dict, Str, Int, Code, SinglefileData
from aiida.plugins import CalculationFactory, DataFactory
from aiida.engine import WorkChain
from pymatgen.core.composition import Composition
from pymatgen.core.periodic_table import Element
from itertools import combinations_with_replacement
from aiida_pyflame.codes.utils import get_min_d, get_element_list, get_allowed_n_atom_for_compositions
...
class GenSymCrysWorkChain(WorkChain):
    ...
    def _construct_builder(parameters):
        ...
        builder.metadata.options.resources = {
                'num_machines': settings.job_script['gensymcrys']['nodes'],
                'num_mpiprocs_per_machine': settings.job_script['gensymcrys']['ntasks']}
        builder.metadata.options.max_wallclock_seconds = settings.job_script['gensymcrys']['time']
        return builder

    @staticmethod
    def _get_additional_parameters(elements, n_elmnt, n_atom):
        min_d_prefactor = settings.inputs['min_distance_prefactor']
...
class AverDistWorkChain(WorkChain):
    ...
    def _construct_builder(parameters, nat):
        ...
        builder.metadata.options.resources = {
                'num_machines': 1,
                'num_mpiprocs_per_machine': settings.job_script['divcheck']['ntasks']}
        builder.metadata.options.max_wallclock_seconds = settings.job_script['divcheck']['time']
        return builder
...
class FLAMETrainWorkChain(WorkChain):
    ...
    def _construct_builder(parameters, cycle_number):
        ...
        builder.metadata.options.resources = {
                'num_machines': settings.job_script['train']['nodes'],
                'num_mpiprocs_per_machine': settings.job_script['train']['ntasks']}
        builder.metadata.options.max_wallclock_seconds = settings.job_script['train']['time']
        return builder
...
class FLAMEMinhocaoWorkChain(WorkChain):
    ...
    def _get_additional_parameters(pymatgen_structure, cycle_number):
        element_list = get_element_list()
        c_no = int(cycle_number.split('-')[-1])
        allowed_n_atom, _ = get_allowed_n_atom_for_compositions(settings.inputs['Chemical_formula'])
...
class FLAMEDivCheckWorkChain(WorkChain):
    ...
    def _construct_builder(parameters, nat, cycle_number, bc):
        ...
        builder.metadata.options.resources = {
                'num_machines': settings.job_script['divcheck']['nodes'],
                'num_mpiprocs_per_machine': settings.job_script['divcheck']['ntasks']}
        builder.metadata.options.max_wallclock_seconds = settings.job_script['divcheck']['time']
        return builder
